...
class Database(DatabaseBase):
    ...
    def load_subject(self,
        ...
        self.data = mne.io.read_raw_bdf(data)# Database load
    ...
    def get_run(self,
        ...
        for i in range(len(events)):
          if ((events[i][2] in right_hemifield_keys) or ((events[i][2] in left_hemifield_keys))):   
            trial_ind[count_1][0] = (events[i][0] + np.floor(fs*trial_range[0])) # Starting point of interest for each trial
            trial_ind[count_1][1] = (events[i][0] + np.ceil(fs*trial_range[1])) # Final point of interest of each trial 
            count_1 += 1     

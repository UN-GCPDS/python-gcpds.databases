...
class DatabaseBase(metaclass=ABCMeta):
    ...
    def format_channels_selectors(self, channels=None):
        ...
        if channels != ALL:
            ...
            channels = list(range(1, len(self.metadata['channels']) + 1))
    ...
    def format_non_class_selector(self, classes):
        ...
        if classes != ALL:
            classes = [
                self.metadata['non_task_classes'].index(cls)
                if isinstance(cls, str)
                else cls
                for cls in classes
            ]
    ...
    def get_metadata(self):
        """"""
        download_metadata(
            os.path.join(self.path, 'metadata'), self.metadata['metadata']
        )
    ...
    def test_integrity(self):
        ...
        if 'runs_training' in self.metadata:
            ...
            try:
                for subj in range(1, self.metadata['subjects'] + 1):
                    if (
                        not self.metadata['subject_training_pattern'](subj)
                        in self.metadata['subject_training_files']
                    ):
                        logging.error(
        ...
        if 'runs_evaluation' in self.metadata:
            ...
            try:
                for subj in range(1, self.metadata['subjects'] + 1):
                    if (
                        not self.metadata['subject_evaluation_pattern'](subj)
                        in self.metadata['subject_evaluation_files']
                    ):
                        logging.error(
